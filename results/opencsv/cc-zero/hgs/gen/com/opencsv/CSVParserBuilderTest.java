...
public class CSVParserBuilderTest {
    ...
    public void testDefaultBuilder() {
        assertEquals(ICSVParser.DEFAULT_SEPARATOR, builder.getSeparator());
        assertEquals(ICSVParser.DEFAULT_QUOTE_CHARACTER, builder.getQuoteChar());
        assertEquals(ICSVParser.DEFAULT_ESCAPE_CHARACTER, builder.getEscapeChar());
        assertEquals(ICSVParser.DEFAULT_STRICT_QUOTES, builder.isStrictQuotes());
        assertEquals(ICSVParser.DEFAULT_IGNORE_LEADING_WHITESPACE, builder.isIgnoreLeadingWhiteSpace());
        assertEquals(ICSVParser.DEFAULT_IGNORE_QUOTATIONS, builder.isIgnoreQuotations());
        assertEquals(CSVReaderNullFieldIndicator.NEITHER, builder.nullFieldIndicator());
        CSVParser parser = builder.build();
        assertEquals(ICSVParser.DEFAULT_SEPARATOR, parser.getSeparator());
        assertEquals(ICSVParser.DEFAULT_QUOTE_CHARACTER, parser.getQuotechar());
        assertEquals(ICSVParser.DEFAULT_ESCAPE_CHARACTER, parser.getEscape());
        assertEquals(ICSVParser.DEFAULT_STRICT_QUOTES, parser.isStrictQuotes());
        assertEquals(ICSVParser.DEFAULT_IGNORE_LEADING_WHITESPACE, parser.isIgnoreLeadingWhiteSpace());
        assertEquals(ICSVParser.DEFAULT_IGNORE_QUOTATIONS, parser.isIgnoreQuotations());
