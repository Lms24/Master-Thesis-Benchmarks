...
public class CsvToBeanTest {
    ...
    public void throwIllegalStateWhenReaderNotProvidedInBuilder() {
        new CsvToBeanBuilder<SimpleAnnotatedMockBean>(null).withType(SimpleAnnotatedMockBean.class).build();
    }

    @Test(expected = IllegalStateException.class)
    @Ignore("Redundant Test Case (identified by JSR)")
    public void throwIllegalStateWhenTypeAndMapperNotProvidedInBuilder() {
        new CsvToBeanBuilder<SimpleAnnotatedMockBean>(new StringReader(TEST_STRING_WITHOUT_MANDATORY_FIELD)).build();
    }

    @Test
    @Ignore("Redundant Test Case (identified by JSR)")
    public void testMinimumBuilder() {
        List<SimpleAnnotatedMockBean> result = new CsvToBeanBuilder<SimpleAnnotatedMockBean>(new StringReader(TEST_STRING)).withType(SimpleAnnotatedMockBean.class).build().parse();
    ...
    public void testMaximumBuilder() throws FileNotFoundException {
        ...
        CsvToBean csvtb = new CsvToBeanBuilder<AnnotatedMockBeanFull>(new FileReader("src/test/resources/testinputmaximumbuilder.csv")).withEscapeChar('?').withFieldAsNull(// default
        CSVReaderNullFieldIndicator.NEITHER).withFilter(new BegToBeFiltered()).withIgnoreLeadingWhiteSpace(false).withIgnoreQuotations(true).withKeepCarriageReturn(// default
        false).withMappingStrategy(map).withQuoteChar('!').withSeparator('#').withSkipLines(1).withStrictQuotes(// default
        false).withThrowExceptions(false).withType(AnnotatedMockBeanFull.class).withVerifyReader(false).build();
    ...
    public void testColumnMappingStrategyWithBuilder() throws FileNotFoundException {
        List<AnnotatedMockBeanFull> result = new CsvToBeanBuilder<AnnotatedMockBeanFull>(new FileReader("src/test/resources/testinputposfullgood.csv")).withSeparator(';').withType(AnnotatedMockBeanFull.class).build().parse();
        assertEquals(2, result.size());
    }

    @Test
    public void testMappingWithoutAnnotationsWithBuilder() {
        List<MockBean> result = new CsvToBeanBuilder<MockBean>(new StringReader(TEST_STRING)).withType(MockBean.class).build().parse();
        assertEquals(2, result.size());
    }
}
