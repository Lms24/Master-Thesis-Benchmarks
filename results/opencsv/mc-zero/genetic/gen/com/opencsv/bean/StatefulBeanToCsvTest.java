...
public class StatefulBeanToCsvTest {
    ...
    private ImmutablePair<AnnotatedMockBeanFull, AnnotatedMockBeanFull> createTwoGoodBeans() throws IOException {
        List<AnnotatedMockBeanFull> beans = new CsvToBeanBuilder(new FileReader("src/test/resources/testinputwriteposfullgood.csv")).withType(AnnotatedMockBeanFull.class).withSeparator(';').build().parse();
        return new ImmutablePair<AnnotatedMockBeanFull, AnnotatedMockBeanFull>(beans.get(0), beans.get(1));
    }

    private ImmutablePair<AnnotatedMockBeanCustom, AnnotatedMockBeanCustom> createTwoGoodCustomBeans() throws IOException {
        List<AnnotatedMockBeanCustom> beans = new CsvToBeanBuilder(new FileReader("src/test/resources/testinputwritecustomposfullgood.csv")).withType(AnnotatedMockBeanCustom.class).withSeparator(';').build().parse();
        return new ImmutablePair<AnnotatedMockBeanCustom, AnnotatedMockBeanCustom>(beans.get(0), beans.get(1));
    }

    private ImmutablePair<AnnotatedMockBeanFullDerived, AnnotatedMockBeanFullDerived> createTwoGoodDerivedBeans() throws IOException {
        ...
        List<AnnotatedMockBeanFullDerived> beans = new CsvToBeanBuilder(new FileReader("src/test/resources/testinputderivedgood.csv")).withType(AnnotatedMockBeanFullDerived.class).withSeparator(';').withMappingStrategy(strat).build().parse();
    ...
    public void writeGoodData() throws IOException, CsvException {
        ...
        StatefulBeanToCsv btcsv = new StatefulBeanToCsv(writer).withQuotechar(CSVWriter.NO_QUOTE_CHARACTER).withSeparator(';');
        ...
        btcsv = new StatefulBeanToCsv(writer).withQuotechar(CSVWriter.NO_QUOTE_CHARACTER).withSeparator(';');
    ...
    public void writeBadData() throws IOException, CsvException, NoSuchFieldException {
        ...
        StringWriter writer = new StringWriter();
        StatefulBeanToCsv sbtcsv = new StatefulBeanToCsv(writer).withQuotechar(CSVWriter.NO_QUOTE_CHARACTER);
        try {
        ...
        // required
        beans.left.setByteWrappedSetLocale(null);
        ...
        assertEquals(beans.left.getClass().getDeclaredField("byteWrappedSetLocale"), rfe.getDestinationField());
        ...
        assertEquals(AnnotatedMockBeanFull.class, rfe.getBeanClass());
        assertEquals(beans.right.getClass().getDeclaredField("dateDefaultLocale"), rfe.getDestinationField());
        // Test case 31
        csves = sbtcsv.getCapturedExceptions();
